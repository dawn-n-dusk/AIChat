...
The Windows package also namespaces connector cursor/receipt state by a stable SHA-256 digest of its trusted local app-server, local-host, channel, and task binding. Its instance-lock metadata is derived from that selected state path; app-server driver receipts keep their independent binding-scoped filenames. An unchanged pre-namespace installation may continue using legacy `state.json`, but changing the fixed mapping creates a new digest-scoped file and never moves or deletes the legacy state.
