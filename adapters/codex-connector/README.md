...
- `text` delivery requires the additional
  ...
  and Windows launchers permit only the narrower `request,result` opt-in and
...
| `AICHAT_DELIVER_TYPES` | no | `request`; core can allow other protocol types, but packaged macOS/Windows accept only `request` or `request,result`. Only `request` is reply-eligible. Adding core-level `text` also requires the autonomy acknowledgement. |
